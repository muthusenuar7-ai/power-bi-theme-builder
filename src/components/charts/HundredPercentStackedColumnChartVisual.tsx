...
export function HundredPercentStackedColumnChartVisual({ showLegend = true, showDataLabels = true, format }: ChartVisualProps) {
        ...
        return (
          <g key={country}>
            {NORMALIZED[ci].map((percentage, si) => {
              const segmentHeight = si === SERIES.length - 1
                ? yCursor - Ly
                : (percentage / 100) * Lh
              ...
              return (
                <g key={si}>
                  ...
                  />
                  {showDataLabels && segmentHeight > 12 && (
                    <text x={x + colW / 2} y={yCursor + segmentHeight / 2 + 2.2}
